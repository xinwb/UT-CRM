...
<div class="page-title">  
    ...
    <div class="breadcrumb-env pull-left">
        ...
        <ol class="breadcrumb bc-1">
            <li>
                <a href="<?php echo url('Category/index'); ?>"><i class="fa-home"></i>首页</a>
            </li>
            <li>
                <a href="">基本设置</a>
             </li>
                    ...
                    <div class="btn-toolbar">
                        ...
                        <form class="form-inline" style="margin-bottom:15px;" class="search-tool">
                            <span>
                                ...
                                <input type="text" class="form-control" placeholder="类别名称" name="name" value="<?php echo \think\Request::instance()->get('name'); ?>"> 
                            </span>
                            
                            <span style="margin-left: 15px;">
                                <label class="control-label">类别编号</label>      
                                <input type="text" class="form-control" placeholder="类别编号" name="sn" value="<?php echo \think\Request::instance()->get('sn'); ?>">
                            </span>
                            ...
                            <span style="margin-left: 15px;">
                                ...
                                <select class="form-control" name="pid">
                                    ...
                                    <?php if(is_array($category) || $category instanceof \think\Collection || $category instanceof \think\Paginator): $i = 0; $__LIST__ = $category;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
                                        <option value="<?php echo $vo['id']; ?>" <?php if($vo['id'] == \think\Request::instance()->get('pid')): ?> selected="selected" <?php endif; ?> ><?php echo $vo['name']; ?></option>
                                    <?php endforeach; endif; else: echo "" ;endif; ?>
                    ...
                    <table class="table table-bordered table-striped" id="example-2">
                        ...
                        <?php if(is_array($list) || $list instanceof \think\Collection || $list instanceof \think\Paginator): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
                            <tr>
                                ...
                                <td> <?php echo !empty($vo->getFather->name)?$vo['getFather']['name'] : '默认'; ?> </td>
                                <td> <?php echo $vo['status']==0?'正常':'废弃'; ?> </td>
                                <td> <?php echo date('Y-m-d H:i:s',$vo->add_time); ?> </td>
                                <td>
                                    <a href="javascript:;" 
                                        class="btn btn-secondary btn-sm btn-icon icon-left" 
                                        onclick="editAjaxModal('<?php echo url('/Category/edit/id/'.$vo->id); ?>')" >
                                        编辑
                                    </a>
                                    
                                    <a ref="javascript:;" class="btn btn-danger btn-sm btn-icon icon-left" onclick="del('<?php echo $vo['id']; ?>')">
                                        删除
                                    </a>
...
<script type="text/javascript">
    ...
    function showAjaxModal(){
        ...
        jQuery.ajax({
            url: "<?php echo url('Category/create'); ?>",
    ...
    function add_from(){
        ...
        $.post("<?php echo url('Category/save'); ?>", res,function(data){
            if( data.error>0 ){
    ...
    function edit_from(){
        ...
        $.post("<?php echo url('Category/update'); ?>", res,function(data){
            if( data.error>0 ){
                toastr.error( data.msg );  
            }else{
                toastr.success( data.msg );  
    ...
    function del(id){
        if( confirm("确定要删除吗？") ){
            $.post("<?php echo url('Category/delete'); ?>", {id:id},function(data){
                if( data.error>0 ){
                    toastr.error( data.msg );  
                }else{
                    toastr.success( data.msg );  
